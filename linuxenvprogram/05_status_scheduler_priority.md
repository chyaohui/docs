...
## 3、普通的优先级
传统的调度算法面临着一种困境，那时间片到底多大才合适？
* 如果时间片太大，进程执行前需要等待的时间就会变长，当 CPU 运行队列上可运行进程的个数较多时尤为明显，用户可能会感觉到明显的延迟。
* 如果时间片太短，进程调度的频率就会增加，考虑到上下文切换也需要花费时间，大量时间都浪费到了进程调度上。

完全公平调度，使用了一种动态时间片的算法，它给每个进程分配了使用 CPU 时间比例。
* 一个很重要的指标是调度延迟，即保证额每一个可运行的进程都至少运行一次时间间隔。
* 比如调度延迟是 20 毫秒，如果运行队列上只有 2 个同等优先级进程，那么允许每个进程执行 10 毫秒
* 如果运行队列上是 4 个同等优先级的进程，那么，每个进程可以运行 5 毫秒

如果运行队列上有 200 个同等优先级的进程，每个进程运行 0.1 毫秒？时间片太小，进程调度过于频繁，上下文切换的开销就不能忽视了。

* 完全公平调度提供了另一种控制方法：调度最小粒度。
* 任何一个进程，只要分配到了 CPU 资源，都会至少执行调度最小粒度的时间，除非进程在执行过程中执行了阻塞的系统调用或主动让出 CPU 资源。

在 Linux 操作系统中，在 sysctl 中可以查看到，单位都是纳秒：
```
# 调度延迟
...
调度延迟和调度最小粒度反映了在调度延迟内允许的最大活动的进程数目，如果运行队列的可运行进程太多，调度最小粒度和调度延迟两个目标则不可能同时实现。调度延迟是一个尽力而为的目标，当运行进程个数小于 sched_nr_latency 时，调度周期总是等于调度延迟；但是如果可运行的进程个数超过 sched_nr_latency，系统就会放弃调度延迟的承诺，转而保证调度最小粒度。


完全公平调度通过引入调度权重来是吸纳优先级，进程之间按照权重的比例，分配 CPU 时间，引入权重后，调取周期内分配给进程的运行时间的计算公式如下：

`分配给进程的运行时间 = 调度周期 * 进程权重 / 运行队列所有进程权重之和`

Linux 下每个进程都有一个 nice 值，该值的取值范围是 [-20, 19]，其中 nice 值越高，表示优先级越低，默认优先级为 0。
```
weight = 1024 / (1.25 ^ nice_value)
```

Linux 提供如下函数来获取和修改进程的 nice 值：
```cpp
int getpriority(int which, int who);
int setpriority(int which, int who, int prio);
```
两个系统调用的前两个参数都是 which 和 who，who 参数如何解释，取决于 which 参数的值，具体如下：
* PRIO_PROCESS：操作进程 ID 为 who 的进程，如果 who 为 0，那么使用调用者的进程 ID。
* PRIO_PGRP：操作进程组 ID 为 who 的进程组的所有成员，如果 who 等于 0，那么使用调用者的进程组 ID
* PRIO_USER：操作所有真实用户 ID 为 who 的进程，如果 who 为 0，使用调用者的真实用户 ID。


关于返回值：
* getpriority 函数返回 which 和 who 指定进程的 nice 值。如果存在多个进程符合指定的标准，那么返回优先级最高的 nice 值。
* 因为进程优先级的范围是 [-20, 19]，所以成功的时候，返回也可能是 -1，因此不能用返回值是不是 -1 来判断是成功还是失败，正确做法是，调用前将 errno 设置为 0，然后调用 getpriority 函数，如果返回值是 -1，并且 errno 不是 0，才能确定调用失败，否则调用成功。
